...
class MainActivity408 : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //후처리 결과는 동일, 과정이 다름
        //구글 측에서, 이 함수를 사용해달라 권고
        val requestLauncher: ActivityResultLauncher<Intent> = registerForActivityResult(

            //이 부분이 시스템에서 제공을 해주고 있음
            //PDF 13.액티비티 컴포넌트 11/56
        ...
        binding.button1.setOnClickListener {
            ...
            //여기서 후처리 함수를 호출, 현재 1번 : 메인 -> 2번 : 디테일
            //2번 디테일 화면에서 데이터를 잘 성정해서 되돌려주면, 위에 정의가 된 ActivityResultContracts.StartActivityForResult()
