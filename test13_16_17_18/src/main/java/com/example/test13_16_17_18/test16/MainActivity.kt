...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        requestPermissionLauncher = registerForActivityResult(
            ...
            //후처리의 결과는 Map 형태의 컬렉션에 담아서 옴 -> it
            //Map 키 값의 형태로 제공
            //아래의 런처 함수가 수행이 되어서 작업이 성공하면 후처리는 여기서 받아서 작업을 진행
            for (entry in it.entries) {
                if(entry.key == "android.permission.READ_CONTACTS" && entry.value) {
                    Log.d("kkang", "granted ok...")
                    val intent =
                        Intent(Intent.ACTION_PICK, ContactsContract.CommonDataKinds.Phone.CONTENT_URI)
                    //인텐트 후처리하는 런처 함수 실행 하면
        ...
        //후처리 2번째 작업, 연락처의 정보 중에서 특정 정보를 가져오는 작업
            ...
            ActivityResultContracts.StartActivityForResult())
        {
            if(it.resultCode == RESULT_OK){
                ...
                //프로바이더로 제공된 데이터에 접근하는 함수중에 하나
                //select name, email from user where email = 'kdy@naver.com' and name='kdy'
