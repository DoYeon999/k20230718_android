...
//주의사항, 미디어 서버에 접근하는 허가 부분이 조금 변경이 되어서 소개 후 사용하고
//콘텐츠 프로바이더 부분의 authorities 부분 주의해서 작업 따라하면 됨
//내용은 그대로 재사용, 코드 리뷰 할 때 설명 잘 보기

//ch16_provider/src/main/java/com/example/ch16_provider/MainActivity.kt
//변경사항
//바인딩 변경 : ActivityImageBinding
//매니페스트 권한 설정
//사이즈 변경 부분에 res-dimen 경로
//SimpleDataFormet 자바 임포트
//data 임포트

//임의의 프로필 사진 한장 준비 교체
//val photoURI: Uri = FileProvider.getUriForFile( 이 부분
//변경하기
...
class ImageActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val requestCameraFileLauncher = registerForActivityResult(
            ...
            //크기를 조정하는 함수
            //calculateInSampleSize 함수 : 저자가 읨의로 만든 원본의 크기를 줄이는 비율은 구하는 식
            ...
            Log.d("kkang", "원본의 사진을 얼마나 줄일 지 비율 값(calRatio) 후처리 $calRatio")
        ...
        binding.cameraButton.setOnClickListener {
            ...
            //JPEG_${timeStamp}_ ,  .jpg 파일준비, 물리 파일
            ...
            //카메라에서 찍은 사진에 접근하기 위해섴 노텐츠 프로바이더에 요청
            //요청시, 매니페스트에서 정한 같은 문자열을 사용
    ...
    //이미지의 크기를 줄이는 로직
    //첫 매개변수 : 사진 원본 데이터
    //두번째, 세번째 매개변수 : 원하는대로 가로 세로 크기(출력 원하는 사진의 크기)
    private fun calculateInSampleSize(fileUri: Uri, reqWidth: Int, reqHeight: Int): Int {
        //비트맵 객체 그대로 사용하면 사진 원본을 그대로 사용해서 메모리 부족 현상 생김
        //그래서 옵션이라는 속성을 이용
        val options = BitmapFactory.Options()
        //실제 비트맵 객체를 생성하는 것 아니고, 옵션만 설정하겠다라는 의미
