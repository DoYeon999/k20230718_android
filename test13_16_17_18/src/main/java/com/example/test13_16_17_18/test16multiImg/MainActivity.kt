...
//https://github.com/lsy3709/K230201AndroidLab/blob/master/test16-2multiimg/src/main/java/com/example/test16_2multiimg/MainActivity.kt
//변경사항
//1 기존 : ActivityMainBinding -> ActivityMain5Binding
//2 MainActivity 뷰에서 속성값 -> tools:context=".test16multiImg.MainActivity">
//3 MultiImageItemBinding : 이미지를 리사이클러 뷰로 출력하기위한 목록 아이템 요소의 뷰
//4 경로 test16-2multiimg/src/main/res/layout/multi_image_item.xml
//5 Glide : 18장에서 소개 할 예정, 이미지를 출력해주는 도구(코루틴 적용, 비동기식처리)
//6 많이 사용하는 도구이다. 설정하려면 build.gradle 모듈 파일에서 디펜던시 추가하면 됨
//7 경로 test16-2multiimg/build.gradle
//필요한 내용은 맨 아래에 디펜던시 안에 있음 ( implementation 'com.github.bumptech.glide:glide:4.12.0' )
//매니페스트에 액티비트 true 변경
class MainActivity : AppCompatActivity() {
    lateinit var binding : ActivityMain5Binding
    //갤러리에서 선택된 이미지의 주소를 하나의 객체로 리스트에 담음
    ...
    class MultiImageAdapter(val items:ArrayList<Uri>,val context: Context) :
        ...
        override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
            ...
            Glide.with(context).load(items[position])
                    //출력 이미지의 가로세로 정할 수 있음
                .override(500,500)
                    //출력 이미지뷰에 선택한 이미지를 불러오는 역할
                .into(binnding.image)
        }
